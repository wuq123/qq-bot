...
def test_load_config_from_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("NOTE_ALLOWED_USER_IDS", "u1, u2, ,u3")
    ...
    assert config.note_allowed_user_ids == ["u1", "u2", "u3"]


def test_empty_note_allowed_user_ids(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("QQBOT_APP_ID", "app-id")
    monkeypatch.setenv("QQBOT_APP_SECRET", "secret")
    monkeypatch.delenv("NOTE_ALLOWED_USER_IDS", raising=False)

    config = BotConfig.from_env()

    assert config.note_allowed_user_ids == []
